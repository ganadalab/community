    ...
    try {
      ...
      if (users.length) {
        ...
        emailClass.create(email, {
          ...
          content: `
          <h1>아이디 찾기 - ${res.locals.setting.siteName}</h1>
          <hr>
          <p>${user.nickName} 님 가입 된 아이디</p><p>${user.uId}</p>`,
        });
        flash.create({
          status: true,
          message: '이메일이 발송되었습니다',
      ...
      if (settings.length) {
        ...
        if (users.length) {
          ...
          emailClass.create(email, {
            ...
            content: `
            <h1>새 비밀번호 - ${res.locals.setting.siteName}</h1>
            <hr>
            <p><a href="${res.locals.setting.siteDomain}/findPassword/newPassword/${hash}">새 비밀번호 생성</a>`,
          });
          flash.create({
            status: true,
            message: '이메일이 발송되었습니다.',
