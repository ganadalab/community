...
exports.new = doAsync(async (req, res, next) => {
  ...
  try {
    ...
    if (method === 'GET') {
      ...
      const articleClass = new Article(req, res, conn);
      const article = await articleClass.get(articleId);
      const {  title, content, html, tags, links, nickName, password, customField01, customField02, customField03, customField04, customField05, customField06, customField07, customField08, customField09, customField10 } = req.body;
