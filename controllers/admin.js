...
exports.log = doAsync(async (req, res, next) => {
  ...
  try {
    ...
    const { today, yesterday, month } = await counter.getCount();
...
exports.userEdit = doAsync(async (req, res, next) => {
  ...
  try {
    ...
    if (submit === 'edit') {
      const { uId, password, nickName, email, phone, permission } = req.body;
  ...
  try {
    ...
    if (method === 'GET') {
      ...
      res.render('admin/userDetail', {
        pageTitle: `${user.uId} (${user.nickName}) 유저 - ${res.locals.setting.siteName}`,
        user,
        userUserGroups,
      });
    } else if (method === 'POST') {
...
exports.boardDetail = doAsync(async (req, res, next) => {
  ...
  try {
    ...
    if (method === 'GET') {
      ...
      const { submit } = req.body;
      if (submit === 'detail') {
        const { writePoint, commentPoint, readPoint, useSecret, useAnonymous, useOnce, useLinks, useFiles, useHtml, useUserGroupPermission, useUserAlarm, useAdminAlarm } = req.body;
        const data = {
          writePoint,
          commentPoint,
          readPoint,
...
exports.settingBasic = doAsync(async (req, res, next) => {
  const conn = await pool.getConnection();
  try {
    const { siteNameRaw, siteName, siteDescription, siteKeywords, siteDomain } = req.body;
    const data = {
      siteNameRaw,
      siteName,
      siteDescription,
...
exports.settingEmail = doAsync(async (req, res, next) => {
  const conn = await pool.getConnection();
  try {
    const { emailService, gmailUser, gmailOauthClientId, gmailOauthClientSecret, gmailOauthRefreshToken } = req.body;
    const data = {
      emailService,
      gmailUser,
      gmailOauthClientId,
...
exports.settingAdsense = doAsync(async (req, res, next) => {
  const conn = await pool.getConnection();
  try {
    const { adsenseIndexTop, adsenseIndexBottom, adsenseSide, adsenseCustom, adsenseArticleTop, adsenseArticleBottom, adsenseArticleCenter, adsenseAds } = req.body;
    const data = {
      adsenseIndexTop,
      adsenseIndexBottom,
      adsenseSide,
...
exports.hidden = doAsync(async (req, res, next) => {
  ...
  try {
    ...
    if (method === 'GET') {
      ...
      const license = req.body.license || 0;
      const data = {
        index,
        useCustomLayout,
        headerLayout,
