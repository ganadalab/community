...
class Board extends Class {
  ...
  async set () {
    status.boards = await this.getBoards();
  }
  async create (data) {
    data = Object.assign({
      title: null,
  ...
  async update (boardId, data) {
    ...
    const { title, slug, type, listCount, listPermission, readPermission, writePermission, commentPermission, writePoint, commentPoint, readPoint, useSecret, useAnonymous, useOnce, useLinks, useFiles, useHtml, useUserGroupPermission, useUserAlarm, useAdminAlarm } = data;
    ...
    const query = `UPDATE board SET title=?, slug=?, type=?, listCount=?, listPermission=?, readPermission=?, writePermission=?, commentPermission=?, writePoint=?, commentPoint=?, readPoint=?, useSecret=?, useAnonymous=?, useOnce=?, useLinks=?, useFiles=?, useHtml=?, useUserGroupPermission=?, useUserAlarm=?, useAdminAlarm=?,
    ...
    await this.conn.query(query, [
      title, slug.trim(), type, listCount, listPermission, readPermission, writePermission, commentPermission, writePoint, commentPoint, readPoint, useSecret, useAnonymous, useOnce, useLinks, useFiles, useHtml, useUserGroupPermission, useUserAlarm, useAdminAlarm,
      useCustomField01, useCustomField02, useCustomField03, useCustomField04, useCustomField05, useCustomField06, useCustomField07, useCustomField08, useCustomField09, useCustomField10,
      customFieldTitle01, customFieldTitle02, customFieldTitle03, customFieldTitle04, customFieldTitle05, customFieldTitle06, customFieldTitle07, customFieldTitle08, customFieldTitle09, customFieldTitle10,
      boardId,
    ]);
