...
const imageUpload = async (image, folder, fileName, size) => {
  try {
    return new Promise(async (resolve, reject) => {
      ...
      if (config.storage === undefined || config.storage === 's3') {
        ...
        if (!folderExist) fs.mkdirSync(path.join(__dirname, `../public/storage/${folder}`), { recursive: true });
        fs.writeFileSync(path.join(__dirname, `../public/storage/${folder}/${key}`), data);
        resolve(key);
      }
    });
  } catch (e) {
